...
if(imageFileInput){
  ...
  imageFileInput.addEventListener('change',async()=>{
    ...
    try{
      ...
      alert('تعذر تجهيز الصورة. جرّب صورة أخرى.');
...
async function loadPosts(){
  ...
  try{
    ...
    for(const key of keys){
      ...
      const snap=await db
        .collection(ADMIN_CATEGORIES[key].collection)
...
$('inquiriesList').addEventListener(
  ...
  async e=>{
    ...
    if(reply){
      ...
      const text=
        card.querySelector('.reply-input').value.trim();
  ...
  async e=>{
    ...
    try{
      ...
      if(selectedImageData){
        ...
        // حذف رابط الصورة القديم إذا كان موجودًا
        data.imageUrl=
