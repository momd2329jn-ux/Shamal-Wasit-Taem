...
async function loadMyInquiries(){
  ...
  try{
    ...
    const unseenReplies=rows.filter(doc=>doc.data().status==='replied'&&doc.data().userSeen!==true);
    if(unseenReplies.length){
      await Promise.all(unseenReplies.map(doc=>db.collection('inquiries').doc(doc.id).update({userSeen:true})));
    }
    box.innerHTML=rows.length?rows.map(doc=>{
      const d=doc.data();
      return `<article class="inquiry-card"><div class="post-meta">${escI(d.type)} ${d.createdAt?'• '+escI(inquiryDate(d.createdAt)):''}</div><h3>${escI(d.title)}</h3><p>${escI(d.message)}</p><div class="inquiry-reply"><strong>رد الفريق</strong><p>${d.reply?escI(d.reply):'لم يتم الرد على هذا الاستفسار بعد.'}</p>${d.repliedAt?`<small>${escI(inquiryDate(d.repliedAt))}</small>`:''}</div></article>`;
