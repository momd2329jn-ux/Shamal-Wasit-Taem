...
async function loadCategoryPage(key, limit = null) {
  ...
  el.innerHTML =
    '<div class="empty-state">جاري تحميل المنشورات...</div>';
  ...
  try {
    ...
    try {
      ...
      console.warn(
        `تعذر استخدام createdAt في ${cfg.collection}`,
        orderError
      );
    ...
    el.innerHTML = docs
      .map(doc => postCard(doc.data(), doc.id))
      .join('');

  } catch (e) {
    console.error(
      `CATEGORY LOAD ERROR: ${key}`,
      e
    );

    el.innerHTML =
      '<div class="empty-state">تعذر تحميل المنشورات حاليًا.</div>';
  }
}
...
async function loadHomeFeeds() {
  ...
  for (const key of Object.keys(CATEGORY_CONFIG)) {
    ...
    const box =
      document.querySelector(`[data-feed="${key}"]`);
    ...
    try {
      ...
      try {
        ...
        snap = await db
          .collection(CATEGORY_CONFIG[key].collection)
          .orderBy('createdAt', 'desc')
          .limit(3)
          .get();
        ...
        console.warn(
          `تعذر ترتيب ${key} بواسطة createdAt`,
          orderError
        );
